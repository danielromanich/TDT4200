...
void renderMeshFractal(
    ...
    // Only render mesh within the index range of this rank
...
std::vector<unsigned char> rasterise(std::vector<Mesh> &meshes, unsigned int width, unsigned int height, unsigned int depthLimit) {
	...
	// We can calcualte the total "nodes" in the rendering "tree" by expanding it by the power of 26 every time
	// This is because for every cube we render, we get 26 new cubes around it in the next depth layer
	int totalRenders = 1;
	for (unsigned int i = 1; i < depthLimit; i++) {
		totalRenders += pow(26, i);
	}

    int range = totalRenders / size;
    int start = rank * range;
    // It is not given that the number of nodes is divisible by the number of ranks. This is solved by giving the last rank
    // the rest of the nodes (the deviation will never be larger than n ranks).
    ...
    MPI_Allreduce(&generalFrameBuffer.front(), &endFrameBuffer.front(), (int) endFrameBuffer.size(), MPI_UNSIGNED_CHAR, MPI_BOR, MPI_COMM_WORLD);
