...
static unsigned int subDiv = 8;

static unsigned int res = 4096;
static unsigned int maxDwell = 1024;
...
int computeSide(std::vector<std::vector<int>> &dwellBuffer,
...
//It is faster for me to run the 4 sides on each threads, because i have more than 4 cores. Since the overhead coming
//from creating/joining threads is smaller than computing each side we get a speedup.
//We might have been slowed down, if the recommended number of threads is below 4. This might cause us to not have proper
//parallelism, meaning our join functions will halt for a long time.
//The slow-down can also be seen if we make the blockDim sufficiently small. If this happens, the overhead of using threads
//is way larger than the time it actually takes to calculate the borders in sequence; resulting is a big slow-down.
				 ...
				 unsigned int const blockSize)
{
	...
	for (unsigned int i = 0; i < 4; i++) {
		threads[i] = std::thread(computeSide, std::ref(dwellBuffer), std::ref(cmin), std::ref(dc), atY, atX, xMax, yMax, blockSize, std::ref(colors[i]), i);
	}
	for (unsigned int i = 0; i < 4; i++) {
		threads[i].join();
	}
	int commonDwell = colors[0];
	for (unsigned int i = 1; i < 4; i++) {
		if (commonDwell != colors[i]) {
			return -1;
		}
	}
	return commonDwell;
...
void threadedComputeBlock(std::vector<std::vector<int>> &dwellBuffer,
	std::complex<double> const &cmin,
	std::complex<double> const &dc,
	unsigned int const startY,
	unsigned int const endY,
	unsigned int const blockSize,
	unsigned int const omitBorder = 0)
{
	for (unsigned int y = startY + omitBorder; y < endY - omitBorder; y++) {
		for (unsigned int x = 0 + omitBorder; x < blockSize; x++) {
			dwellBuffer.at(y).at(x) = pixelDwell(cmin, dc, y, x);
		}
	}
}
...
int main( int argc, char *argv[] )
{
	...
	if (mariani) {
		...
		std::cout << "Corrected blocksize " << correctedBlockSize << std::endl;
		// Mariani-Silver subdivision algorithm
		if (thread) {
			unsigned int const nThreads = std::thread::hardware_concurrency();
			std::thread *threads = new std::thread[nThreads];

			unsigned int const bestFit = std::floor(std::sqrt(nThreads));
			std::cout << "Best fit: " << bestFit << " * 2" << std::endl;
			unsigned int newBlockSize = correctedBlockSize / bestFit;
			std::cout << "Block: " << newBlockSize << std::endl;
			int counter = 0;
			for (unsigned int i = 0; i < bestFit; i++) {
				for (unsigned int j = 0; j < bestFit; j++) {
					threads[counter] = std::thread(marianiSilver, std::ref(dwellBuffer), std::ref(cmin), std::ref(dc),
												   i * newBlockSize, j * newBlockSize, correctedBlockSize / bestFit);
					counter++;
				}
			}
			for (unsigned int i = 0; i < bestFit * bestFit; i++) {
				threads[i].join();
			}
		} else {
		...
		if (thread) {
			unsigned int const nThreads = std::thread::hardware_concurrency();
			std::thread *threads = new std::thread[nThreads];
			int const range = res / nThreads;
			for (unsigned int i = 0; i < nThreads; i++) {
				int startY = range * i;
				int endY = i == nThreads - 1 ? res : range * (i + 1);
				threads[i] = std::thread(threadedComputeBlock, std::ref(dwellBuffer), std::ref(cmin), std::ref(dc), startY, endY, res, 0);
			}
	...
	// 6580 no Thread, 1780 with threads (average from 5 runs).
	//
