...
static unsigned int blockDim = 32;
static unsigned int subDiv = 8;

static unsigned int res = 1024;
static unsigned int maxDwell = 256;
...
void marianiSilver( Job job)
{
    int dwell = commonBorder(job.dwellBuffer, job.cmin, job.dc, job.atY, job.atX, job.blockSize);
    if ( dwell >= 0 ) {
        fillBlock(job.dwellBuffer, dwell, job.atY, job.atX, job.blockSize);
        if (mark)
            markBorder(job.dwellBuffer, dwellFill, job.atY, job.atX, job.blockSize);
    } else if (job.blockSize <= blockDim) {
        computeBlock(job.dwellBuffer, job.cmin, job.dc, job.atY, job.atX, job.blockSize);
        if (mark)
            markBorder(job.dwellBuffer, dwellCompute, job.atY, job.atX, job.blockSize);
    } else {
        // Subdivision
        unsigned int newBlockSize = job.blockSize / subDiv;
        for (unsigned int ydiv = 0; ydiv < subDiv; ydiv++) {
            for (unsigned int xdiv = 0; xdiv < subDiv; xdiv++) {
                Job newJob = {job.dwellBuffer, job.cmin, job.dc, job.atY + (ydiv * newBlockSize), job.atX + (xdiv * newBlockSize), newBlockSize};
                addWork(newJob);
            }
        }
    }
...
void worker() {
    while (true) {
        std::unique_lock<std::mutex> lock(mutex);
        idleCount++;
        cond_var.wait(lock, []{ return !deque.empty() || idleCount == threadCount; });
        if (idleCount == threadCount && deque.empty()) {
            cond_var.notify_all(); // We notify all of the waiting threads since we know that we are done rendering
            break;
        }
        Job job = deque.front();
        deque.pop_front();
...
int main( int argc, char *argv[] )
{
    ...
    if (mariani) {
        ...
        Job baseJob = {dwellBuffer, cmin, dc, 0, 0, correctedBlockSize};
        std::cout << baseJob.blockSize << std::endl;
        for (int i = 0; i < threadCount; i++) {
            threads.push_back(std::thread(worker));
        }
        marianiSilver(baseJob);
